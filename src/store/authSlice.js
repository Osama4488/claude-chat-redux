...
import Cookies from "js-cookie";
import posthog from "../lib/posthog";
import { fetchHistory } from "../utils/authUtils"; // Moved fetchHistory to authUtils.js
...
export const login = createAsyncThunk(
  ...
  async ({ email, password }, { rejectWithValue }) => {
    try {
      ...
      // Save the access token in secure cookies
      ...
      // Encrypt and save the email in secure cookies
      // const encryptedEmail = CryptoJS.AES.encrypt(email, process.env.NEXT_PUBLIC_SECRET_KEY).toString();
      Cookies.set("encryptedEmail", email, {
        expires: 7,
        secure: true,
        sameSite: 'Strict',
        path: '/',
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setUserHistory(state, action) {
      state.user.userHistory = action.payload;
    },
  },
  extraReducers: (builder) => {
    builder
